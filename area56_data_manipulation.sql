...
-- Show first and last names associated with foreign keys in clients table

SELECT clients.clientID, clients.name, clients.address, clients.email, clients.phone, contacts.contactID, contacts.fname, contacts.lName, sales_representatives.repID, sales_representatives.fname, sales_representatives.lname FROM clients 
INNER JOIN contacts on clients.contactID = contacts.contactID 
INNER JOIN sales_representatives on clients.repID = sales_representatives.repID 
ORDER BY clientID;

--get an order table filled with the rep and client names included
            SELECT orders.orderID, orders.repID, CONCAT(sales_representatives.fname,' ',sales_representatives.lname) AS rep, orders.clientID, clients.name
                    FROM orders
...
--get a table filled with the rep and client names included where a search has been conducted for a rep.
            SELECT orders.orderID, orders.repID, CONCAT(sales_representatives.fname,' ',sales_representatives.lname) AS rep, orders.clientID, clients.name
                    FROM orders
                    INNER JOIN sales_representatives on orders.repID = sales_representatives.repID
                    INNER JOIN clients on orders.clientID = clients.clientID
                    WHERE orders.repID = :repID
                    ORDER BY orderID
                    ;

--get a table filled with the rep and client names included where a search has been conducted for client.
            SELECT orders.orderID, orders.repID, CONCAT(sales_representatives.fname,' ',sales_representatives.lname) AS rep, orders.clientID, clients.name
                    FROM orders
                    INNER JOIN sales_representatives on orders.repID = sales_representatives.repID
                    INNER JOIN clients on orders.clientID = clients.clientID
                    ...
                    ORDER BY orderID
                    ;
--get a table filled with the rep and client names included where a search has been conducted for an order.
            SELECT orders.orderID, orders.repID, CONCAT(sales_representatives.fname,' ',sales_representatives.lname) AS rep, orders.clientID, clients.name
                FROM orders
                INNER JOIN sales_representatives on orders.repID = sales_representatives.repID
                INNER JOIN clients on orders.clientID = clients.clientID
...
--get a table to represent the M:M relationship that also hones in on just one order
        SELECT order_products.orderID, order_products.productID, products.item, order_products.quantity, products.cost AS item_cost, order_products.quantity * products.cost AS total_cost
                FROM order_products
...
--get a table for one order line
        SELECT order_products.orderID AS orderID, order_products.productID AS productID, products.item AS item, products.cost AS cost, order_products.quantity AS quantity, order_products.quantity*products.cost AS total_cost
            FROM order_products                
            INNER JOIN products ON order_products.productID = products.productID
            WHERE order_products.orderID = :orderID AND :order_products.productID = :productID};
